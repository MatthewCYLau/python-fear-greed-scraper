...
resource "google_project_iam_member" "analysis_job_pubsub_subscribe" {
  ...
  condition {
    title       = "resource_name_equals_analysis_jobs_subscription_or_orders_or_transactions_subscription"
    description = "Resource name equals ${google_pubsub_subscription.analysis_jobs.name} or ${google_pubsub_subscription.order.name} or ${google_pubsub_subscription.transaction.name}"
    expression  = "resource.name == 'projects/${var.project}/subscriptions/${google_pubsub_subscription.analysis_jobs.name}' || resource.name == 'projects/${var.project}/subscriptions/${google_pubsub_subscription.order.name}' || resource.name == 'projects/${var.project}/subscriptions/${google_pubsub_subscription.transaction.name}'"
  }
}
